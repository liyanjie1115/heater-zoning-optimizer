...
MODULE_PITCH = MODULE_LENGTH + MODULE_GAP   # 安装节距(mm)

OUTER_EDGE_ALLOW = 10.0       # 左右最外边缘各允许超出的长度(mm)

MIN_SIZE = MODULE_LENGTH
EXPORT_FILENAME = "分区结果_灯管间距10mm_边缘允许外超完整版.xlsx"
...
def calc_max_fittable_modules(zone_size, is_first_zone=False, is_last_zone=False,
    ...
    规则：
    1. 内部分区：安装长度 <= 分区长度
    2. 首分区：允许向左外边缘超出 outer_edge_allow
    3. 末分区：允许向右外边缘超出 outer_edge_allow
    4. 不允许出现0模块，因此最少返回1
    """
...
def evaluate_zoning_quality(dist, temp, zones):
    ...
    for z in zones:
        ...
        # 误差定义为：分区长度与实际安装长度的差值绝对值
        # 对首末分区，如果发生外超，不计为内部违规，只反映为边缘外超
        install_length = z["install_length"]
        size = z["size"]

        if z["is_first"] or z["is_last"]:
            # 首末区允许外超 OUTER_EDGE_ALLOW
...
def plot_compare_equal_vs_aligned(dist, temp, zones_equal, zones_aligned, edges, metrics_equal, metrics_aligned):
    ...
    ax = axes[0]
    ax.plot(dist, temp, 'o-', color='black', linewidth=1.2, markersize=4)
    for z in zones_equal:
        x0, x1 = z["range"]
        y = z["avg_temp"]
        xm = 0.5 * (x0 + x1)

        ax.hlines(y, x0, x1, linewidth=4, color='tab:blue')
    ...
    for z in zones_aligned:
        ...
        ax.hlines(y, x0, x1, linewidth=4, color='tab:red')
        ax.axvline(x0, linestyle='--', alpha=0.25, color='gray')
    ...
    plt.suptitle("连续等距分区 与 严格模块对齐分区 对比（内部不超、两端允许外超）", fontsize=14)
    plt.tight_layout()
    plt.show()
